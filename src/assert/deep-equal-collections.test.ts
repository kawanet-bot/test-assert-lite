...
describe(TITLE, () => {
    ...
    it("compares Set/Map contents order-independently, by deep equality", () => {
        assert.doesNotThrow(() => TAL_strict.deepEqual(new Set([1, 2]), new Set([2, 1])))
        assert.throws(() => TAL_strict.deepEqual(new Set([1, 2]), new Set([1, 3])), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual(new Set([1, 2]), new Set([1])), /deep-equal/)
        // Deep, not just ===: object elements/keys/values are matched by content.
        assert.doesNotThrow(() => TAL_strict.deepEqual(new Set([{a: 1}]), new Set([{a: 1}])))

        assert.doesNotThrow(() => TAL_strict.deepEqual(
            new Map([["a", 1], ["b", 2]]),
            new Map([["b", 2], ["a", 1]]),
        ))
        assert.throws(() => TAL_strict.deepEqual(new Map([["a", 1]]), new Map([["a", 2]])), /deep-equal/)
        assert.doesNotThrow(() => TAL_strict.deepEqual(new Map([[{k: 1}, "v"]]), new Map([[{k: 1}, "v"]])))
    ...
    it("keeps -0/+0 and NaN semantics correct through the Set/Map fast path", () => {
        ...
        assert.doesNotThrow(() => TAL_strict.deepEqual(new Set([-0]), new Set([0])))
        assert.doesNotThrow(() => TAL_strict.deepEqual(new Set([NaN]), new Set([NaN])))
        // Map values are not deduplicated, so -0 vs +0 must still differ here.
        assert.throws(() => TAL_strict.deepEqual(new Map([["k", -0]]), new Map([["k", 0]])), /deep-equal/)
        assert.doesNotThrow(() => TAL_strict.deepEqual(new Map([["k", NaN]]), new Map([["k", NaN]])))
    })
    ...
    it("only treats WeakMap/WeakSet/Promise as equal by reference", () => {
        assert.doesNotThrow(() => {
            ...
            TAL_strict.deepEqual(shared, shared)
        })
        const key = {}
        assert.throws(() => TAL_strict.deepEqual(new WeakMap([[key, 1]]), new WeakMap([[key, 1]])), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual(new WeakSet([key]), new WeakSet([key])), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual(Promise.resolve(1), Promise.resolve(1)), /deep-equal/)
    })
})
