...
describe(TITLE, () => {
    ...
    it("deepStrictEqual/notDeepStrictEqual are aliases of deepEqual/notDeepEqual", () => {
        assert.equal(TAL_strict.deepStrictEqual, TAL_strict.deepEqual)
        assert.equal(TAL_strict.notDeepStrictEqual, TAL_strict.notDeepEqual)
    })

    it("compares primitives with Object.is semantics, like equal", () => {
        assert.doesNotThrow(() => TAL_strict.deepEqual(NaN, NaN))
        assert.throws(() => TAL_strict.deepEqual(0, -0), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual(null, undefined), /deep-equal/)
        assert.doesNotThrow(() => TAL_strict.deepEqual("x", "x"))
    })

    it("compares arrays element by element", () => {
        assert.doesNotThrow(() => TAL_strict.deepEqual([1, "a", true], [1, "a", true]))
        assert.throws(() => TAL_strict.deepEqual([1, 2], [1, 2, 3]), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual([1, 2], [2, 1]), /deep-equal/)
    })
    ...
    it("compares nested objects recursively", () => {
        assert.doesNotThrow(() => TAL_strict.deepEqual({a: {b: [1, 2]}}, {a: {b: [1, 2]}}))
        assert.throws(() => TAL_strict.deepEqual({a: {b: [1, 2]}}, {a: {b: [1, 3]}}), /deep-equal/)
    })

    it("requires the same own enumerable keys, not merely the same count", () => {
        assert.throws(() => TAL_strict.deepEqual({a: 1, b: 2}, {a: 1, c: 2}), /deep-equal/)
        // A key explicitly set to undefined is still present and must match.
        assert.throws(() => TAL_strict.deepEqual({a: 1, b: undefined}, {a: 1}), /deep-equal/)
        ...
        assert.throws(() => TAL_strict.deepEqual({a: 1}, b), /deep-equal/)
    })
    ...
    it("requires the same prototype", () => {
        ...
        assert.throws(() => TAL_strict.deepEqual(new Foo(), {a: 1}), /deep-equal/)
        assert.doesNotThrow(() => TAL_strict.deepEqual(new Foo(), new Foo()))
    })
    ...
    it("is not fooled by an own Symbol.toStringTag claiming to be an Array", () => {
        ...
        assert.throws(() => TAL_strict.deepEqual(fakeArray, [1]), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual([1], fakeArray), /deep-equal/)
        ...
        assert.throws(() => TAL_strict.deepEqual([], disguised), /deep-equal/)

        assert.doesNotThrow(() => TAL_strict.deepEqual(new Proxy([1, 2], {}), [1, 2]))
    })
    ...
    it("is not fooled by a real array hiding behind an Object tag", () => {
        ...
        assert.throws(() => TAL_strict.deepEqual(hidden(1), lookalike), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual(lookalike, hidden(1)), /deep-equal/)

        const stretched = hidden(1, 2)
        stretched.length = 5
        assert.throws(() => TAL_strict.deepEqual(hidden(1, 2), stretched), /deep-equal/)
        assert.doesNotThrow(() => TAL_strict.deepEqual(hidden(1, 2), hidden(1, 2)))
    })
    ...
    it("checks array length even when no extra index became enumerable", () => {
        const stretched = [1, 2]
        stretched.length = 5
        assert.throws(() => TAL_strict.deepEqual(stretched, [1, 2]), /deep-equal/)
    })
    ...
    it("does not mistake a plain object's own length property for an array's", () => {
        ...
        assert.doesNotThrow(() => TAL_strict.deepEqual(new Sized(), new Sized()))
    ...
    it("breaks mutually-referencing cycles the same way", () => {
        ...
        b1.other = b2
        b2.other = b1
        assert.doesNotThrow(() => TAL_strict.deepEqual(a1, b1))
    })
    ...
    it("tells apart a 1-cycle from a same-shaped 2-cycle instead of stack-overflowing", () => {
        ...
        y1.self = y2
        y2.self = y1
        assert.throws(() => TAL_strict.deepEqual(x1, y1), /deep-equal/)
    })
    ...
    it("tells apart a self-cycle from a path that merely leads into one, in either order", () => {
        ...
        assert.throws(() => TAL_strict.deepEqual(loop, intoLoop), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual(intoLoop, loop), /deep-equal/)
        ...
        assert.throws(() => TAL_strict.deepEqual(loop, wrapsOther), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual(wrapsOther, loop), /deep-equal/)
        ...
        assert.throws(() => TAL_strict.deepEqual(selfSet, wrapsOtherSet), /deep-equal/)
        assert.throws(() => TAL_strict.deepEqual(wrapsOtherSet, selfSet), /deep-equal/)
    })
    ...
    it("still accepts a shared reference against equal copies, and aliased cycles", () => {
        ...
        assert.doesNotThrow(() => TAL_strict.deepEqual([{inner: 0}, {inner: 0}], [shared, shared]))
        assert.doesNotThrow(() => TAL_strict.deepEqual([shared, shared], [{inner: 0}, {inner: 0}]))

        const inner: Record<string, unknown> = {}
        inner.loop = inner
        const outer: Record<string, unknown> = {loop: inner}
        const actual: Record<string, unknown> = {loop: outer}
        assert.doesNotThrow(() => TAL_strict.deepEqual(actual, outer))
        assert.doesNotThrow(() => TAL_strict.deepEqual(outer, actual))
    })

    it("notDeepEqual is the exact negation", () => {
        assert.doesNotThrow(() => TAL_strict.notDeepEqual([1, 2], [1, 3]))
        assert.throws(() => TAL_strict.notDeepEqual([1, 2], [1, 2]), /expected not to deep-equal/)
    })
    ...
    it("deepEqual keeps the values alongside a custom message", () => {
        const error = catchError(() => TAL_strict.deepEqual([1], [2], "blah"))
        assert.match(String(error?.message), /^blah\n\nexpected \[2] to deep-equal \[1]$/)
        assert.equal(catchError(() => TAL_strict.notDeepEqual([1], [1], "blah"))?.message, "blah")
    })

    it("an Error passed as message is thrown as is", () => {
        const sentinel = new Error("sentinel")
        assert.equal(catchError(() => TAL_strict.deepEqual(1, 2, sentinel)), sentinel)
        assert.equal(catchError(() => TAL_strict.notDeepEqual(1, 1, sentinel)), sentinel)
    })

    it("AssertionError carries actual, expected and the strict operator name", () => {
        const error = catchError(() => TAL_strict.deepEqual({a: 1}, {a: 2})) as Error & {
